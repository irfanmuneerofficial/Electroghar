{!! Theme::partial('header-meta') !!}
    <body style="background-color: #fdf5f5;" @if (Theme::get('pageId')) id="{{ Theme::get('pageId') }}" @endif @if (BaseHelper::isRtlEnabled()) dir="rtl" @endif>
        ...
        <header class="header header--2" style="background-color: white;" data-sticky="{{ Theme::get('stickyHeader', theme_option('sticky_header_enabled', 'yes') == 'yes' ? 'true' : 'false') }}">
            <div class="header__top"style="background-color: white;" >
                <div class="ps-container align-items-center">
                    @if (is_plugin_active('ecommerce'))
                        <div class="header__left">
                            <div class="menu--product-categories" >
                                <div class="menu__toggle" ><i class="icon-menu" style="color:var(--color-1st);"></i><span style="color:var(--color-1st);" > {{ __('Shop by Department') }}</span></div>
                        ...
                        <div class="header__center">
                            <form class="ps-form--quick-search" style="border: 1px var(--color-1st) solid;" action="{{ route('public.products') }}" data-ajax-url="{{ route('public.ajax.search-products') }}" method="get">
                        ...
                        @if (is_plugin_active('ecommerce'))
                            <ul class="navigation__extra">
                                @if (is_plugin_active('marketplace'))
                                    <li><a href="{{ !auth('customer')->check() ? route('customer.register') : (auth('customer')->user()->is_vendor ? route('marketplace.vendor.dashboard') : route('marketplace.vendor.become-vendor')) }}">{{ theme_option('sell_on_site_text') ?: __('Sell On Electroghar') }}</a></li>
                                @endif
                                            ...
                                            <ul class="ps-dropdown-menu">
                                                @foreach ($currencies as $currency)
                                                    @if ($currency->id !== get_application_currency_id())
                                                        <li><a style="color:black" href="{{ route('public.change-currency', $currency->title) }}"><span>{{ $currency->title }}</span></a></li>
                                                    @endif
                                                @endforeach
                                            </ul>
                                        </div>
